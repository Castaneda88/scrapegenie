...
export default async function handler(req, res) {
  ...
  try {
    ...
    $('a[href]').each((_, el) => {
      ...
      if (link && /(product|item|detail)/i.test(link)) {
    ...
    for (const link of limitedLinks) {
      ...
      const varietyList = $$('select option').map((_, o) => $$(o).text().trim()).get();
      const variety = varietyList.length ? varietyList.join(', ') : 'No variety';

      items.push({
        name: name || 'Unnamed Product',
        price: price || 'N/A',
        variety,
        priceByVariety: variety // extend logic here if prices differ
      });
    }
